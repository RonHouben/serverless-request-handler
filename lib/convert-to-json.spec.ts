...
describe("convert-to-json", () => {
  it("should throw an error when invalid json is provided", () => {
    expect(() => convertToJson()).toThrow(
      new HttpError(StatusCodes.BAD_REQUEST, "Invalid json")
    );
    expect(() => convertToJson("")).toThrow(
      new HttpError(StatusCodes.BAD_REQUEST, "Invalid json")
    );
    ...
    expect(() => convertToJson("{ foo: bar }")).toThrow(
      new HttpError(StatusCodes.BAD_REQUEST, "Invalid json")
    );
  });
  ...
  it("should log the error when a logger is provided", () => {
    ...
    expect(() =>
      convertToJson("", mockLogger as any as winston.Logger)
